...
; strict: LFAE-Value -> LFAE-Value[excluding exprV]
; purpose: to force expression closures to values in several places in the interpreter
(define (strict v)
...
; lookup: symbol DefrdSub -> LFAE
; purpose: to look up the value of identifiers in the repository
...
; interp: LFAE ds ->LFAE-Value
; purpose: to evaluate LFAE expression by delay of argument expressions
...
; Why is the expression, {- {+ 4 5} {+ 8 9}}, computed several times? (This is because the symbol x is repeatedly computed each time it is called. In this equation, x is calculated four times in total.)
